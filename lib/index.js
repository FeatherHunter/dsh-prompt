 ...
 * 失败同时落一条 update.route.fail（reason 记机器码，错误原文只留 8 位指纹）。
 */
async function updateRoutes(req, res, path, method, logCap, updateReady) {
  ...
  const error = failed
    ...
    : null;
  if (failed) {
    // 「更新没成」这件事以前只有回包、没有日志（#39 审查 F9）。reason 记机器码（能力缺席 /
    // check-expired 之类），错误原文只以 8 位指纹落盘。
    logCap?.log?.("update.route.fail", { route: path, reason: error.code, errorHash: hashText(error.message) });
  }
...
export function apply(ctx, config = {}) {
  ...
  async function logUpdateFail(reason, message) {
    try {
      ...
      cap?.log?.("update.route.fail", { route: "/_dsh/dsh-prompt/update", reason, errorHash: hashText(message) });
