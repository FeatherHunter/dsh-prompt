...
/**
 * 更新能力的三条路由（#39）。
 * 返回 undefined 表示「这条 URL 不归更新能力管」，调用点继续往下匹配既有端点 ——
 * 这样更新路由只在需要时插一句，既有路由表的顺序与内容都不动。
 * 非 POST / 非白名单路径一律回 undefined，不让更新能力抢走别的端点的 404 语义。
 */
...
export function apply(ctx, config = {}) {
  ...
  const updateReady = (async () => {
    try {
      ...
      return await mod.createUpdateCapability({ ctx, logReady });
    } catch (e) {
      return null;
    }
  })();
