...
DeepSeek Harness 的对话输入框左侧有一个 **⚡Prompt 按钮**。点开是 prompt 模板面板——**点一下，模板正文就插入当前对话输入框**，不用再记忆、搜索、看文档、复制粘贴。
...
- **自定义模板**：＋ 新增、删除仅限自建且二次确认、可编辑；预制可一键「复制为自定义」再改。
- **/prompt 触发源**：在输入框直接输入 `/prompt`（可继续输入关键词过滤，如 `/prompt 思考框架`、`/prompt 拆解`），回车或点选即插入。
- **v1.1 智能悬浮卡**：全局一个低调圆点，可拖动（位置记忆）；输入内容匹配模板时从圆点向右上展开推荐卡，点整行即填入，光标自动定位到第一个字段冒号后；不捕获键盘，Enter 发送不受影响。
- **场景标签**：每个模板带 领域×阶段×动作 三维标签；面板按阶段分组 + 领域筛选 + 搜索。
- **双语界面**：面板跟随 DSH 界面语言（中文 / English）。
- **主题跟随**：字体、字号、颜色全部跟随 DSH 主题与排版设置。
- **本地持久化**：自定义模板、用量计数（仅排序不显示）、置顶、悬浮卡位置与开关都保存在本地。

## /prompt —— 输入即达

![prompt trigger](assets/prompt-trigger-zh.svg)

输入 `/prompt` 弹出模板清单，继续输入即过滤（`/prompt 思考框架`、`/prompt 执行前`、`/prompt 拆解`……按 名称/正文/领域/阶段/动作/标签 检索），回车或点击即插入。

## 智能悬浮卡 —— 匹配即出

![smart card](assets/smart-card-zh.svg)

输入「帮我做个决策，两个选项很纠结」这类内容时，右下角圆点向右上展开推荐卡（≤3 条候选：top-2 智能匹配 + 最近使用，评分 = 专属词×2 + 通用词×1，≥2 分才出卡，宁缺毋滥）。点整行即填入，正文插入后**光标定位到第一个字段的冒号后**，直接开写。
...
**输入框直接输入 `/prompt`** → 斜杠菜单列出模板（含自定义），继续输入即过滤，回车 / 点击选中即插入。

**智能模式（默认开）**：匹配输入时出卡；点 × 收起回到圆点；继续输入可再次出现；可在设置里关闭。
...
- 客户端源码：`src/client/*`（面板 / 触发源 / 智能匹配 / 词表 / 桥接 / 设置页）
...
合适的切入点：新预制模板（冒号表单式）、词表打磨、UI 细节、文档与翻译。提 Issue / PR 均可，欢迎！
