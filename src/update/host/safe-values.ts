 ...
 *
 * 为什么必须有这一层：白名单只回答「这个字段能不能进」，闸门那五条具名规则只管 token /
 * Windows 绝对路径 / 家目录 / URL / 邮箱这五种形状，`method` / `kind` / `route` / `reason`
 * 在闸门里都是**自由字符串**（清单里的 `codes:["ENUM"]` 只是文档，运行时不执行）——
 * 于是「提示词正文与模板名绝不进日志」只剩「调用点恰好都塞字面量」这层人为约定，
 * 复审 D 把 47 字符正文与模板名逐字写进了真日志文件。
 *
