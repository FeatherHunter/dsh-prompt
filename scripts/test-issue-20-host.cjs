...
// 覆盖：路由注册、空快照、put 校验（body≤10000）、bump 原子计数+lastUsed、pinned 上限与对账、
...
(async () => {
  ...
  // put 边界（#72：10000 通过、10001 → 400）
  out = await call('POST', '/_dsh/dsh-prompt/customs/put', { template: { ...tpl, id: 'c10000', body: 'x'.repeat(10000) } });
  eq(out.status, 200, '10000 字 body 应通过');
  out = await call('POST', '/_dsh/dsh-prompt/customs/put', { template: { ...tpl, id: 'cbad', body: 'x'.repeat(10001) } });
  eq(out.status, 400, '超长 body 应 400');
  ok('schema 校验（body≤10000）拒绝非法写入');
  ...
  out = await call('POST', '/_dsh/dsh-prompt/customs/delete', { id: 'c10000' });
  eq(out.status, 200, 'delete 边界模板状态码');
  out = await call('GET', '/_dsh/dsh-prompt/store');
