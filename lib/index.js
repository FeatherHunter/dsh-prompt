...
/* ── 更新路由失败的「值域安全网 + 按状态落一次」（#39 三轮整改 H1 / H2）─────────────────
 * H1：字段**名**的权威在真闸门（清单白名单），但闸门对白名单内的**字符串值**只管五种具名形状
 * （token / Windows 绝对路径 / 家目录 / URL / 邮箱）——`reason` 在闸门里是自由字符串，包改口径或
 * 升版就能把一整段正文落盘（复审 D 实测：47 字符正文进了真日志文件）。本文件这条 update.route.fail
 * 的 reason 直接取自包回包的 error 字段，属于同一类「值来自上下文」的面，所以按与桥
 * （src/update/host/index.ts 的 SAFE_VALUE_RE）**同一条规则**兜底：不匹配安全字符集的字符串换成
 * 8 位指纹。**不判字段名** —— 白名单只有真闸门那一套，别在桥前面再立一道。
 * H2：同一 (route, reason) 只在状态第一次出现时落一条 —— 电话持续失败时这是与「能力缺席」同一量级的
 * 刷屏面（复审 D 实测 62 次请求 124 行 / 20402 B ⇒ 54.2 MiB/天），而 warn 绕过日志开关、用户关不掉。
 * 两条纪律：① 首条必须落（真实故障不许因为去重而看不见）；② 同一条路由成功一次即清账 ——
 * 恢复之后再失败是**新状态**，重新落一条。账本按 updateReady（每个宿主实例一份）分开，免得
 * 两个宿主实例串状态。 */
const SAFE_FIELD_RE = /^[A-Za-z0-9._:\/-]{0,64}$/;
const ROUTE_FAIL_SEP = "\u0000";
const routeFailLedger = new WeakMap();

/** 单值兜底：非字符串原样（类型归闸门管），字符串不匹配安全字符集就换指纹。 */
function safeFieldValue(value) {
  if (typeof value !== "string") return value;
  return SAFE_FIELD_RE.test(value) ? value : hashText(value);
}

/** true = 这个失败状态第一次出现（该落盘）；false = 同一状态已经落过。 */
function noteRouteFail(updateReady, route, reason) {
  let seen = routeFailLedger.get(updateReady);
  if (!seen) {
    seen = new Set();
    routeFailLedger.set(updateReady, seen);
  }
  const key = String(route) + ROUTE_FAIL_SEP + String(reason);
  if (seen.has(key)) return false;
  seen.add(key);
  return true;
}

/** 这条路由成功了：清掉它的失败账（下一次失败是新状态，重新落一条）。 */
function clearRouteFails(updateReady, route) {
  const seen = routeFailLedger.get(updateReady);
  if (!seen) return;
  const prefix = String(route) + ROUTE_FAIL_SEP;
...
async function updateRoutes(req, res, path, method, logCap, updateReady) {
  ...
  if (failed && !capabilityAbsent) {
    ...
    // 错误原文只以 8 位指纹落盘；同一 (route, reason) 只在状态首次出现时落（#39 三轮整改 H2），
    // 值另过安全网、正文形状的 reason 换成指纹（H1）。
    if (noteRouteFail(updateReady, path, error.code)) {
      logCap?.log?.("update.route.fail", {
        route: safeFieldValue(path),
        reason: safeFieldValue(error.code),
