  ...
  const handler = async (req, res) => {
    ...
    try {
      ...
      if (method === "POST" && path === "/_dsh/dsh-prompt/debug") {
        ...
        // 删除动作在地图 #45 的打点票（#50）里做，届时现场信息改由 smart.probe 事件承担。
        const body = await readJsonBody(req);
        ...
        try {
          ...
          logCap?.log?.("host.route.fail", {
            route: "/_dsh/dsh-prompt/debug",
            method,
            status: 500,
            errorHash: hashText(String(e?.message || e)),
          });
        }
        logCap?.log?.("host.route.fail", { route: "/_dsh/dsh-prompt/debug", method, status: 200, errorHash: hashText("client-diag") });
