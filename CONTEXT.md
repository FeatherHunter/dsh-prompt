...
每个模板被插入使用的累计次数。仅作面板排序依据，不向用户展示数字；悬浮列表按用量升序从上往下排（最常用在底部），设置页仍按用量降序从上往下排。
...
悬浮列表的排序方式：最常用模板在列表最底部最先看到，向上依次为第 2、3、4…多，未使用在最顶部；置顶模板集中在底部最贴近按钮的区域，簇内仍按用量升序，同分按预制原始顺序或自定义创建时间。
