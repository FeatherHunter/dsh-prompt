		...
		* 文本≈宿主 clipboardText（段落↔\n，空段落即空行）；光标=当前 selection 锚点，无选择即末尾。 */
		function readEditable(root) {
			...
			try {
				...
				let curTop = -1;
				const pushRun = (s, node, top) => {
					if (!s) return;
					runs.push({
						node,
						start: text.length,
						len: s.length,
						top: typeof top === "number" ? top : curTop
					});
					text += s;
				};
				if (!kids.some((k) => k && k.nodeType === 1)) pushRun(root.textContent || "", null, -1);
				else {
					const sep = () => {
					...
					const walkList = (list) => {
						for (const k of list) {
							...
							let ch = [];
							try {
								ch = k.childNodes ? Array.prototype.slice.call(k.childNodes) : [];
							} catch (e) {
								ch = [];
							}
							if (BLOCK_TAGS[tag] === 1) {
								...
								walkList(ch);
								if (text.length === before) forceBreak();
					...
					if (an && root.contains) {
						...
						if (inside) {
							...
							if (an.nodeType === 3) {
								...
								let hit = -1;
								for (const r of runs) if ((typeof r.top === "number" && r.top >= 0 ? r.top : r.node ? kids.indexOf(r.node) : -1) >= idx) {
									hit = r.start;
									break;
								}
								...
								let hitEl = r.node === an;
								if (!hitEl) try {
									hitEl = !!an.contains && an.contains(r.node);
								} catch (e) {
									hitEl = false;
								}
								if (hitEl) {
		...
		function resolveDraft() {
			...
			if (root) {
				...
				if (stripNewlines(H) === stripNewlines(d)) {
					const tailOnly = (a, b) => a.length > b.length && a.indexOf(b) === 0 && /^\n*$/.test(a.slice(b.length));
					if (tailOnly(H, d) || tailOnly(d, H)) return {
						text: H,
						caret: Math.max(0, Math.min(c, H.length))
					};
					...
					if (countNewlines(H) > countNewlines(d)) {
						...
						let j = 0;
						let kept = 0;
						while (j < H.length && kept < wantKept) {
							if (H.charCodeAt(j) !== 10) kept++;
							j++;
						}
						return {
							text: H,
							caret: Math.max(0, Math.min(j, H.length))
						};
					}
				}
