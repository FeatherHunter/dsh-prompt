...
每个模板被插入使用的累计次数。仅作排序依据，不向用户展示数字（显数由用量任务落地前保持）；悬浮列表分区内按用量升序从上往下排（最常用在底部），智能卡同分时按用量升序；设置页与 /prompt 保留用量降序且忽略置顶（管理面与菜单首屏例外，置顶仅悬浮列表生效 #68）。
...
统一的选择类列表排序方向：最优先的模板在列表最底部最先看到，向上依次递减。悬浮列表以置顶分区为一级键（置顶簇在底部连续聚拢不散插）、分区内以用量为键，智能卡以评分为键（最相关在底部）；悬浮置顶簇内同用量时按 pin 顺序，仍相同时按预置原始顺序或自定义创建时间。设置页与 /prompt 为明确例外，保留用量降序且忽略置顶。
